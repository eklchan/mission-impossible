...
const App: React.FunctionComponent = () => {
    ...
    const handleSwipeResult = (profile: Profile, result: boolean) => {
        ...
        const updatedStack: Array<Profile> = stack.filter((prof, i) => {
    ...
    const renderStack = stack.map((profile, i) => {
        const topCard: boolean = i === stack.length - 1;

        return (
            <CardItem
                drag={topCard}
                profile={profile}
                handleSwipeResult={handleSwipeResult}
                key={i}
                likeState={likeState}
                    ...
                    <div className="results-wrap">
                        ...
                        <div className="table-wrap">
                            <div className="rejected">
                                ...
                                {rejected.map((reject) => {
                                    return <h1 key={reject.name}>{reject.name}</h1>;
                                })}
                            </div>
                            <div className="accepted">
                                <img src={acceptedIcon} alt="accepted" className="results-icon"></img>
                                {accepted.map((accept) => {
                                    return <h1 key={accept.name}>{accept.name}</h1>;
                                })}
                            </div>
                        </div>
                        <button className="again-button" onClick={handleReset}>
                            Start Again 🔄
                        </button>
                    </div>
                )}
                {!!stack.length && (
                    <div className="button-wrap">
                        <button className="button" onClick={handleReject}>
                            <img src={cross} alt="cross" className="button-image" />
                        </button>
                        <button className="button" onClick={handleAccept}>
                            <img src={tick} alt="tick" className="button-image" />
                        </button>
                    </div>
                )}
            </div>
